...
export class WalletConfigService {
  ...
  getWalletConfig(payload: any): WalletConfigData {
    ...
    this.logger.debug(`Extracting wallet config for blockchain: ${blockchainName}, token: ${tokenSymbol}, walletId: ${walletId}`);
    ...
    this.logger.log(`Determined wallet name: ${walletName} for blockchain: ${blockchainName}, token: ${tokenSymbol}`);
    
    // Get configuration from environment variables based on wallet name
    const config = this.configService.get(walletName);
    
    if (!config) {
      this.logger.warn(`No configuration found for wallet name: ${walletName}, using blockradar configuration`);
    ...
    if (walletId && config.walletId && walletId !== config.walletId) {
      this.logger.warn(`Wallet ID mismatch: ${walletId} (payload) vs ${config.walletId} (config), using payload wallet ID`);
    }
  ...
  getBlockchainName(payload: any): string {
    ...
    const blockchainName = payload?.data?.blockchain?.name || payload?.data?.network || 'ethereum';
