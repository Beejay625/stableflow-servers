...
function loadEnvironment() {
  ...
  if (!fs.existsSync(envPath)) {
    console.error(`[WORKER ENV] ERROR: Environment file ${envPath} not found. Worker cannot start.`);
    process.exit(1);
  }
...
async function bootstrap() {
  ...
  logger.debug('[WORKER] Initializing application (services, repositories, and connections)');
  await app.init();
  ...
  try {
    ...
    if (dataSource) {
      ...
      logger.log(`[WORKER] Database connection established via URL: ${sanitizedUrl}`);
      logger.log(`[WORKER] Connection pool size: ${dataSource.options.extra?.max || 'default'}`);
      logger.log(`[WORKER] Connection status: ${dataSource.isInitialized ? 'Initialized' : 'Not initialized'}`);
    }
  } catch (err) {
  ...
  try {
    ...
    if (redisService) {
      ...
      logger.log(`[WORKER] Redis connection status: ${redisService.isConnected() ? 'Connected' : 'Not connected'}`);
  ...
  try {
    ...
    if (queueService) {
      ...
      logger.log(`[WORKER] Active workers: ${await queueService.getWorkerCount()}`);
    }
  } catch (err) {
  ...
  logger.log('[WORKER] Worker is using the same connection pool as the API server');
