...
async function testTokenBalances() {
  ...
  console.log(
    `Fetching balances for Wallet ID: ${walletId}, Address ID: ${addressId}`,
  );
  console.log(
    `Using network from .env file: ${process.env.NETWORK || "not set"}`,
  );
  ...
  try {
    ...
    if (tokenIdArgs.length > 0) {
      console.log(
        `Using custom token IDs from command line: ${tokenIdArgs.join(", ")}`,
      );
      ...
      supportedTokens.forEach((token) => {
        console.log(
          `- ${token.tokenSymbol} on ${token.blockchainName} (ID: ${token.tokenId})`,
        );
      });
    }
    ...
    if (tokenIds.length === 0) {
      console.log(
        "Warning: No token IDs found. Check if TokenService is configured correctly.",
      );
    }
    ...
    if (!allTokens.data || !allTokens.data.data) {
      ...
      if (tokenDetails.length > 0) {
        ...
        tokenDetails.forEach((token: any) => {
          console.log(
            `- ${token.symbol} on ${token.blockchain.name} (ID: ${token.id})`,
          );
          console.log(`  Address: ${token.address}`);
        });
      } else {
        console.log(
          "Warning: None of the specified token IDs were found in the API",
        );
      }
    }
    ...
    for (const balance of balances) {
      ...
      console.log(
        `Found token: ${symbol} on ${blockchainName} (ID: ${tokenId})`,
      );
    ...
    if (tokenBalances.length === 0) {
      ...
      console.log("\n===== SUMMARY =====");
      const summary = tokenBalances.map(
        (t) =>
