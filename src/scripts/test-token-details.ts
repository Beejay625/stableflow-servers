...
const tokenSymbols = process.argv
  .slice(2)
  .map((symbol) => symbol.toUpperCase());
console.log(
  `Filtering for tokens: ${tokenSymbols.length > 0 ? tokenSymbols.join(", ") : "All supported stablecoins"}`,
);
...
const getTokenDetails = async (
  ...
  try {
    ...
    const blockchainNames = [
      ...new Set(tokens.map((token: any) => token.blockchain?.name)),
    ];
    const availableSymbols = [
      ...new Set(tokens.map((token: any) => token.symbol?.toUpperCase())),
    ];
    ...
    if (filteredTokens.length > 0) {
      ...
      filteredTokens.forEach((token: any, index: number) => {
        console.log(
          `${index + 1}. ${token.name} (${token.symbol}) on ${token.blockchain?.name} (Network: ${token.network})`,
        );
      });
    }
