...
async function testWalletGeneration() {
  ...
  if (!businessId) {
    ...
    console.error('Usage: npx ts-node -r tsconfig-paths/register src/scripts/test-wallet-generation.ts <businessId> [force]');
    console.error('  - businessId: ID of the business to test wallet generation');
    console.error('  - force: Optional "force" parameter to force wallet regeneration even if business already has one');
    process.exit(1);
  }

  console.log(`Testing wallet generation for business: ${businessId}${forceOption ? ' (force mode)' : ''}`);
  ...
  try {
    ...
    if (business.walletDetails && !forceOption) {
      ...
      console.log('Example: npx ts-node -r tsconfig-paths/register src/scripts/test-wallet-generation.ts <businessId> force');
      process.exit(0);
    }
    ...
    if (isReady) {
      ...
      // Generate wallet
      console.log('\nGenerating wallet...');
      const result = await walletService.generateWalletForCompletedBusiness(businessId);
      ...
      console.log(`Is EVM compatible: ${result.data.blockchain?.isEvmCompatible}`);
