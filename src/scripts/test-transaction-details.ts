...
const walletId =
  process.argv[3] || process.env.BLOCKRADAR_WALLET_ID || process.env.WALLET_ID;
...
if (!walletId) {
  console.error(
    "Wallet ID must be provided as an argument or in BLOCKRADAR_WALLET_ID or WALLET_ID environment variable",
  );
  process.exit(1);
}
...
async function fetchTransactionDetails() {
  console.log(
    `Fetching details for Transaction ID: ${transactionId} from Wallet ID: ${walletId}`,
  );
  ...
  try {
    ...
    console.log(
      `Token: ${extractedDetails.tokenName} (${extractedDetails.tokenSymbol})`,
    );
    console.log(
      `Blockchain: ${extractedDetails.blockchainName} (${extractedDetails.blockchainSymbol})`,
    );
