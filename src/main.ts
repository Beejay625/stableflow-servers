  ...
  if (enableClustering && (cluster as any).isMaster) {
    ...
    try {
      ...
      if (environment === "DEVELOPMENT") {
        ...
        logger.log(
          "Staging environment - using test data and staging services",
        );
      ...
      try {
        ...
        if (!inUse) {
          ...
          logger.warn(
            `Port ${configuredPort} is already in use, searching for an available port...`,
          );
        ...
        logger.log(
          `Webhook endpoint: ${await app.getUrl()}/api/v1/wallet/webhook/blockradar`,
        );
