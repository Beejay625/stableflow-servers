...
import { AuthResponseDto, BusinessAuthResponseDto, TokenRefreshResponseDto } from './dto';
...
export class AuthService {
  ...
  async verifyOtpWithBusinessId(email: string, otp: string): Promise<BusinessAuthResponseDto> {
    ...
    if (!user) {
      this.logger.warn(`Authentication failed: User with email ${normalizedEmail} not found`);
    ...
    if (!storedOtp || storedOtp !== otp) {
      this.logger.warn(`Authentication failed: Invalid or expired OTP for ${normalizedEmail}`);
    ...
    if (!business) {
      ...
      businessId = business.id;
      this.logger.log(`Found existing business with ID ${businessId} for user ${user.id}`);
    }
    
    // Calculate token expiration time
    const expiresIn = parseInt(this.configService.get('JWT_EXPIRATION_SECONDS') || '86400', 10);
    ...
    const refreshExpiresIn = parseInt(this.configService.get('JWT_REFRESH_EXPIRATION_SECONDS') || '604800', 10); // Default 7 days
   ...
   */
  async refreshToken(refreshToken: string, accessToken: string): Promise<TokenRefreshResponseDto> {
    ...
    try {
      ...
      if (!accessUserId || !accessEmail) {
        this.logger.warn('Invalid access token payload: missing userId or email');
      ...
      if (!userId || !email) {
        this.logger.warn('Invalid refresh token payload: missing userId or email');
        throw new UnauthorizedException('Invalid refresh token');
      }
      
      // Verify that the user IDs and emails match between both tokens
      if (userId !== accessUserId || email !== accessEmail) {
        this.logger.warn(`Token mismatch: refresh token user (${userId}, ${email}) doesn't match access token user (${accessUserId}, ${accessEmail})`);
      ...
      const shouldRotateToken = this.configService.get('JWT_REFRESH_ROTATION') === 'true';
