...
export class PaycrestService {
  ...
  async getInstitutions(currencyCode?: string): Promise<Institution[]> {
    try {
      ...
      if (currencyCode) {
        const response = await lastValueFrom<AxiosResponse<PaycrestResponse<Institution[]>>>(
          this.httpClient.get<PaycrestResponse<Institution[]>>(`institutions/${currencyCode}`, {
            baseURL: this.baseUrl,
            headers: this.headers,
            timeout: DEFAULT_TIMEOUT,
          }).pipe(retryWithBackoff(DEFAULT_RETRY_ATTEMPTS, DEFAULT_TIMEOUT)) as Observable<AxiosResponse<PaycrestResponse<Institution[]>>>
        );
  ...
  async getExchangeRate(data: ExchangeRateRequest): Promise<PaycrestResponse<any>> {
    try {
      ...
      const response = await lastValueFrom<AxiosResponse<PaycrestResponse<any>>>(
        this.httpClient.post<PaycrestResponse<any>>('exchange-rate', data, {
          baseURL: this.baseUrl,
          headers: this.headers,
          timeout: DEFAULT_TIMEOUT,
        }).pipe(retryWithBackoff(DEFAULT_RETRY_ATTEMPTS, DEFAULT_TIMEOUT)) as Observable<AxiosResponse<PaycrestResponse<any>>>
      );
  ...
  async getTokenRate(
    ...
    try {
      ...
      const response = await lastValueFrom<AxiosResponse<PaycrestResponse<any>>>(
        this.httpClient.get<PaycrestResponse<any>>(`rates/${token}/${amount}/${fiat}${providerId ? `/${providerId}` : ''}`, {
          baseURL: this.baseUrl,
          headers: this.headers,
          timeout: DEFAULT_TIMEOUT,
        }).pipe(retryWithBackoff(DEFAULT_RETRY_ATTEMPTS, DEFAULT_TIMEOUT)) as Observable<AxiosResponse<PaycrestResponse<any>>>
