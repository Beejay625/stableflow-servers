...
export class TokenService {
  ...
  constructor(
    ...
    this.logger.log(
      `TokenService initialized with default network: ${this.defaultNetwork}`,
    );
  }
  ...
  async getStablecoinTokens(
    ...
    try {
      ...
      const effectiveNetworks =
        networks && networks.length > 0 ? networks : [this.defaultNetwork];
      ...
      const filteredTokens = tokens.filter((token: any) => {
        ...
        const networkMatches =
          token.network && effectiveNetworks.includes(token.network);
      ...
      const symbolFilterText =
        symbols && symbols.length > 0
          ? symbols.join(", ") + " tokens"
          : "all supported stablecoins";

      const blockchainFilterText =
        blockchains && blockchains.length > 0
          ? blockchains.join(", ") + " blockchains"
          : "supported blockchains";

      this.logger.debug(
        `Filtered to ${filteredTokens.length} tokens (${symbolFilterText} on ${blockchainFilterText} for ${networkFilterText})`,
      );
