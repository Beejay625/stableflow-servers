...
export class WalletController {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve transactions associated with a specific business with pagination",
  })
  @ApiQuery({
    name: "page",
    required: false,
  ...
  async getProcessingTransactions() {
    const processingTransactions =
      await this.transactionQueue.getProcessingTransactions();
    return {
  ...
  async getFailedTransactions() {
    const failedTransactions =
      await this.transactionQueue.getFailedTransactions();
    return {
