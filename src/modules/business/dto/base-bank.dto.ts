import { IsNotEmpty, IsString, Length, ValidateIf, Matches } from 'class-validator';
...
export class BaseBankDto {
  ...
  @ApiProperty({ description: 'Bank code (do not provide if using bankName)', required: false })
  bankCode?: string;
  ...
  @Length(2, 100)
  @Matches(/^[a-zA-Z\s\-&]+$/, {
    message: 'Bank name can only contain letters, spaces, hyphens, and ampersands'
  })
  @ApiProperty({ description: 'Bank name (do not provide if using bankCode)', required: false })
  bankName?: string;
  ...
  @IsNotEmpty()
  @IsString()
  @Length(10, 10, { message: 'Nigerian bank account numbers must be exactly 10 digits' })
  @Matches(/^[0-9]{10}$/, {
