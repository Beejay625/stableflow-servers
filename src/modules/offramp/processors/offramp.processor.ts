...
export class OfframpProcessor {
   ...
   */
  @Process('process')
  async processOfframp(job: Job<{ transactionId: string; status: TransactionStatus }>) {
    ...
    try {
      ...
      if (result.status !== TransactionStatus.UNSETTLED) {
        ...
        const updatedTransaction = await this.transactionRepository.findOne({ where: { transactionId } });
        const errorMessage = updatedTransaction?.metadata?.offramp?.errors?.[0]?.message || 'Unknown error during offramp';
