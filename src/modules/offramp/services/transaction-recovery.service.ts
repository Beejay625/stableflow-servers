...
export class TransactionRecoveryService {
  ...
  async recoverUnprocessedTransactions() {
    ...
    try {
      ...
      if (unprocessedTransactions.length === 0) {
        this.logger.debug(
          "No unprocessed transactions found requiring recovery.",
        );
        return;
      }

      this.logger.log(
        `Found ${unprocessedTransactions.length} potential transactions to recover.`,
      );
      ...
      for (const tx of unprocessedTransactions) {
        ...
        if (!isQueued) {
          ...
          alreadyQueuedCount++;
          this.logger.debug(
            `Transaction ${tx.transactionId} already in queue, skipping.`,
          );
        }
      }
      ...
      this.logger.error(
        `Error in transaction recovery: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async checkStalledTransactions(): Promise<void> {
    try {
      ...
      const stalledTransactions = await this.transactionRepository
        ...
        .andWhere("transaction.receivedAt < :threshold", {
      ...
      this.logger.log(
        `Found ${stalledTransactions.length} potentially stalled transactions`,
      );

      for (const transaction of stalledTransactions) {
        try {
          const result = await this.offrampService.checkAndRecoverTransaction(
            transaction.id,
          );

          if (result.recovered) {
            this.logger.log(
              `Successfully recovered stalled transaction ${transaction.id}`,
            );
          } else if (result.status === "pending_confirmation") {
            this.logger.debug(
              `Transaction ${transaction.id} is still pending confirmation`,
            );
          } else {
            this.logger.warn(
              `Could not recover transaction ${transaction.id}: ${result.message}`,
            );
          }
        } catch (error) {
          this.logger.error(
            `Error processing stalled transaction ${transaction.id}: ${error.message}`,
            error.stack,
          );
        }
      }
      ...
      this.logger.error(
        `Error checking stalled transactions: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async manualRecovery(olderThanMinutes: number = this.recoveryAgeMinutes) {
    this.logger.log(
      `Manual recovery triggered for transactions older than ${olderThanMinutes} minutes.`,
    );
