...
export class TransactionQueueService {
  ...
  async getProcessingTransactions(): Promise<any[]> {
    try {
      ...
      this.logger.error(
        `Error fetching processing transactions: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async getFailedTransactions(): Promise<any[]> {
    try {
      ...
      this.logger.error(
        `Error fetching failed transactions: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async retryTransaction(transactionId: string): Promise<boolean> {
    try {
      ...
      const job = failedJobs.find(
        (job) => job.data.transactionId === transactionId,
      );

      if (!job) {
        this.logger.warn(
          `Failed job for transaction ${transactionId} not found`,
        );
        return false;
      }
      ...
      this.logger.error(
        `Error retrying transaction ${transactionId}: ${error.message}`,
        error.stack,
      );
      return false;
    }
  ...
  async isTransactionQueued(transactionId: string): Promise<boolean> {
    try {
      ...
      if (
        waitingJobs.find((job) => job.data?.transactionId === transactionId)
      ) {
        return true;
      }
      ...
      return !!delayedJobs.find(
        (job) => job.data?.transactionId === transactionId,
      );
    } catch (error) {
      this.logger.error(
        `Error checking if transaction ${transactionId} is queued: ${error.message}`,
      );
      return false;
    }
  }
